...
import { ProductItem, ClassifyImageResponse, PredictionModel, SpecData, ActionType, ImagePickerType } from '../../models'
...
interface ReviewScreenState {
    loading: boolean;
    detectedObjects: Array<ProductItem>;
    imageSource: any;
    specData?: SpecData,
    tags: Array<any>;
    editRegionButtonStyle: ButtonStyle;
    removeRegionButtonStyle: ButtonStyle;
    clearRegionsButtonStyle: ButtonStyle;
}
...
export class ReviewScreen extends React.Component<ReviewScreenProps, ReviewScreenState> {
    ...
    async componentDidMount() {
        ...
        let imageProps = navigation.getParam('image', {});
        let specData = navigation.getParam('selectedSpec', {});
        let imagePickerType = navigation.getParam('imagePickerType', ImagePickerType.FromWeb);

        this.imageFileName = imageProps?.fileName ?? '';
        let imageSrc = imagePickerType === ImagePickerType.FromWeb ? imageProps : imageProps.uri;
        this.setState({ imageSource: imageSrc });
        const data = await this.analyzeImage(specData.ModelId, imageSrc, imagePickerType);
    ...
    async analyzeImage(projectId: string, imageSrc: any, imagePickerType: ImagePickerType) {
        let result = new ClassifyImageResponse();
        try {
            this.setState({ loading: true });
            
            const iterations = await this.customVisionService.getIterationsAsync(projectId);
            result = await this.analyzeImageByIteration(iterations, imageSrc, imagePickerType);
    ...
    async analyzeImageByIteration(data: any, imageSrc: any, imagePickerType: ImagePickerType) {
        let result = new ClassifyImageResponse();
        ...
        if (data && data.length) {
            const filteredData = data.filter((item: any) => {
              ...
              if (latestTrainedIteraction === null || !latestTrainedIteraction.publishName) {
                ...
                const projectId = latestTrainedIteraction.projectId;
                const publishName = latestTrainedIteraction.publishName;

                switch(imagePickerType) {
                    case ImagePickerType.FromWeb:
                        await this.customVisionService.detectImageUrl(projectId, publishName, imageSrc)
                            .then((responseJson) => {
                                result = responseJson;
                            })
                            .catch((error) => {
                    ...
                    case ImagePickerType.FromImageGallery:
                        let file = {
                            name: 'data',
                            filename: this.imageFileName || 'image.jpg',
                            type: 'image/jpeg',
                            data: RNFetchBlob.wrap(imageSrc),
                        };
                        await this.customVisionService.detectImageFromFile(projectId, publishName, file)
                            .then((responseJson) => {
                                result = responseJson;
                            })
                            .catch((error) => {
                    ...
                    case ImagePickerType.FromCamera:
                        await this.customVisionService.detectImageFromCameraPicture(projectId, publishName, imageSrc)
                            .then((responseJson) => {
                                result = responseJson;
                            })
                            .catch((error) => {
    ...
    onRegionSelectionChanged(selectedCount: number) {
        this.setState({
